/**
 * \file libs/numeric/ublasx/test/exp.cpp
 *
...
BOOST_UBLASX_TEST_DEF( test_real_vector )
{
	...
	BOOST_UBLASX_DEBUG_TRACE( "exp(v) = " << res );

	for (size_type i = 0; i < n; ++i)
	{
		expect_res(i) = std::exp(v(i));
	}
...
BOOST_UBLASX_TEST_DEF( test_real_matrix )
{
	...
	BOOST_UBLASX_DEBUG_TRACE( "exp(A) = " << R );

	for (size_type r = 0; r < nr; ++r)
	{
		for (size_type c = 0; c < nc; ++c)
		{
			expect_R(r,c) = std::exp(A(r,c));
		}
	}
...
int main()
{
	...
	BOOST_UBLASX_DEBUG_TRACE("Test Suite: 'exp' operation");
