...
/**
 * \file libs/numeric/ublasx/test/log10.cpp
 *
 * \brief Test suite for the \c log10 operation.
 *
...
BOOST_UBLASX_TEST_DEF( test_real_vector )
{
    ...
    BOOST_UBLASX_DEBUG_TRACE( "log10(v) = " << res );

    for (size_type i = 0; i < n; ++i)
    {
        expect_res(i) = std::log10(v(i));
    }
...
BOOST_UBLASX_TEST_DEF( test_real_matrix )
{
    ...
    BOOST_UBLASX_DEBUG_TRACE( "log10(A) = " << R );

    for (size_type r = 0; r < nr; ++r)
    {
        for (size_type c = 0; c < nc; ++c)
        {
            expect_R(r,c) = std::log10(A(r,c));
        }
    }
...
int main()
{
    ...
    BOOST_UBLASX_DEBUG_TRACE("Test Suite: 'log10' operation");
